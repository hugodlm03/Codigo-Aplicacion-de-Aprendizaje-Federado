...
def main(cfg: DictConfig) -> None:
    ...
    if cfg.centralized:
        if cfg.dataset.dataset_name == "all":
            ...
            out_dir.mkdir(exist_ok=True) # type: ignore
            csv_path = out_dir / "results_centralized.csv"

            create_res_csv(csv_path, writer.fields)
            writer.write_res(
                csv_path,
        ...
        # ─── ruta de salida ──────────────────────────────────────────
        out_dir = Path(__file__).parent / "results"        # hfedxgboost/results/
